...
exports.run = async (client, message, args, option) => {
  ...
  if (!message.member.voice.channel)
    return message.reply("You need to join to voice channel 📶")
  ...
  async function play(connection, message) {
    ...
    message.channel.send(
      new MessageEmbed()
        ...
        .setColor(0x00ccff)
        .setDescription(
          `▶ ${songInfo.videoDetails.title} requested by ${message.member.nickname}`
        )
    )
    ...
    server.dispatcher.on("finish", () => {
      ...
      if (server.queue[0]) {
        ...
        setTimeout(() => {
          if (!client.voice.connections.size && !server.queue[0]) {
